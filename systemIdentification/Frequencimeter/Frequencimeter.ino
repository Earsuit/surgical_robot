...
void loop(){ 
	if(t<RUNTIME){
		...
		if(output){
			...
			vel.data = encoderPulseLocal ? -(DEGREES_PER_PULSE*countLocal/dt) : DEGREES_PER_PULSE*countLocal/dt;
...
void pinChangeSetup(){
	EIMSK = 0x00;
	EICRB = _BV(ISC50) | _BV(ISC51);	//rising edge cause an interrupt
	EIFR = 0xFF;
	EIMSK = _BV(INT5);		//interrupt enable
}
