...
void publisherCallback(MiniSerial&,uint8_t*,ros::Publisher&,surgical_robot::motor_feedback &,const ros::TimerEvent&);
...
int main(int argc, char** argv){
    ...
    pub_callback publisher_callback =  boost::bind(publisherCallback,boost::ref(serial),buffer,boost::ref(motor_feedback_pub),boost::ref(msg),_1);
    ros::Timer timer = n.createTimer(ros::Duration(rate),publisher_callback);

    //subscriber
    sub_callback sub_callback = boost::bind(subscriberCallback,boost::ref(serial),_1);
...
void subscriberCallback(MiniSerial &serial,const surgical_robot::motor_commandsConstPtr &msg){
    ROS_DEBUG("Commands received: %d, %d, %d, %d, %d, %d, %d, %d", msg->motor_1_v,msg->motor_1_dir,msg->motor_2_v,msg->motor_2_dir,msg->motor_3_v,msg->motor_3_dir,msg->motor_4_v,msg->motor_4_dir);
...
void publisherCallback(MiniSerial& serial,uint8_t* buffer,ros::Publisher& pub,surgical_robot::motor_feedback & msg,const ros::TimerEvent& timer){
    serial.flush();
    ...
    ROS_DEBUG("%f, %f, %f, %f",msg.motor_1,msg.motor_2,msg.motor_3,msg.motor_4);
    pub.publish(msg);
}